...
class CJKSegmenter implements ISegmenter {
    ...
    CJKSegmenter( Granularity granularity )
    {
        this.tmpHits = new LinkedList<Hit>();
    ...
    public void analyze(AnalyzeContext context) {
        if(CharacterUtil.CHAR_USELESS != context.getCurrentCharType()){
            ...
            if(singleCharHit.isMatch()){//首字成词
                ...
                //同时也是词前缀
                if( singleCharHit.isPrefix() && isDefaultGranularity() )
                {
                    //前缀匹配则放入hit列表
                    this.tmpHits.add( singleCharHit );
                }
            }
            else if( singleCharHit.isPrefix() && isDefaultGranularity() )
            {//首字为词前缀
                //前缀匹配则放入hit列表
                this.tmpHits.add( singleCharHit );
            }
    ...
    private boolean isDefaultGranularity()
    {
        return granularity == Granularity.Default;
    }
